...
def recommend(user,test_data,communities,network,userVecs,n_hops, top_20=False):
...
if __name__ == '__main__':
	...
	for alg_type in alg_types:
		...
		with Pool(5) as p:
			results = p.starmap(recommend,zip(most_active_users,repeat(test_data),repeat(communities),repeat(network),repeat(userVectors),repeat(n_hops),repeat(top_20)))

		for user, user_data in results:
			final_data[user] = user_data

		if top_20:
			fn = 'results/user_ranking/top_20/{}_hops/{}.json'.format(n_hops,alg_type)

		else: 
			fn = 'results/user_ranking/total_set/{}_hops/{}.json'.format(n_hops,alg_type)
